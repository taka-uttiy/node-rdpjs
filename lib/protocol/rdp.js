...
function RdpClient(config) {
	...
	this.rdpdr = new pdu.rdpdr.Client(this.sec);
	this.rdpsnd = new pdu.rdpsnd.Client(this.sec);
	...
	if (config.audio) {
		var svchid = t125.mcs.Channel.MCS_GLOBAL_CHANNEL + 1;
        // rdpdr : https://learn.microsoft.com/en-us/openspecs/windows_protocols/ms-rdpea/deaaab30-d4d6-473e-ac56-3f76c4f6fb24#Appendix_A_1 <1>
        this.mcs.channels.push({id : svchid ++, name : 'rdpdr', flags : (t125.gcc.ChannelOptions.CHANNEL_OPTION_INITIALIZED  | t125.gcc.ChannelOptions.CHANNEL_OPTION_COMPRESS_RDP) >>> 0 });
        // rdpsnd
        this.mcs.channels.push({id : svchid ++, name : 'rdpsnd', flags : (t125.gcc.ChannelOptions.CHANNEL_OPTION_INITIALIZED | t125.gcc.ChannelOptions.CHANNEL_OPTION_ENCRYPT_RDP | t125.gcc.ChannelOptions.CHANNEL_OPTION_PRI_HIGH) >>> 0 });

		this.mcs.createNetworkData()
	}
	...
	case 'fr':
		...
		break;
	case 'ja':
		log.debug('japanese keyboard layout');
		this.mcs.clientCoreData.obj.kbdLayout.value = t125.gcc.KeyboardLayout.JAPANESE;
		break;
	case 'en':
	default:
	...
	// bind rdpsnd events
	this.rdpsnd.on('audio', function (audio) {
		//console.log("** audio ");
		//console.log(audio);
		self.emit('audio', audio);
	});
