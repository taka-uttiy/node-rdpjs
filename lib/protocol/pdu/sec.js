...
Sec.prototype.send_channel = function(channel, message) {
...
Sec.prototype.recv_channel = function(name, s) {
	// not support yet basic RDP security layer
	this.emit(name, s);
};
/**
...
Sec.prototype.recvLicense = function(s) {
    ...
    if (message.bMsgtype.value === lic.MessageType.NEW_LICENSE || 
		...
		this.transport.on('rdpdr', function(s) {
			self.recv_channel('rdpdr', s);
		});
		this.transport.on('rdpsnd', function(s) {
			self.recv_channel('rdpsnd', s);
		});
		this.transport.on('drdynvc', function(s) {
			self.recv_channel('drdynvc', s);
		});
    	return;
    }
