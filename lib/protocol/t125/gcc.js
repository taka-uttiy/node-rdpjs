 ...
 */
var ClusterFlags = {
	CC_REDIRECTION_SUPPORTED : 0x00000001,
	CC_REDIRECT_SESSIONID_FIELD_VALID : 0x00000002,
	CC_REDIRECTED_SMARTCARD : 0x00000040,
	CC_REDIRECT_VERSION_MASK : 0x0000003c
};
var ClusterVersion = {
	CC_REDIRECT_VERSION_3 : 0x02,
	CC_REDIRECT_VERSION_4 : 0x03,
	CC_REDIRECT_VERSION_5 : 0x04,
	SEC_CC_REDIRECT_VERSION_6 : 0x05
};
	...
	var self = {
	    ...
	    data : data || new type.Factory(function(s){
	    	...
	    	case MessageType.CS_NET:
	    		...
	    		break;
            case MessageType.CS_CLUSTER:
				self.data = clientClusterData(0, options).read(s);
				break;
	    	default:
 ...
 * @returns {type.Component}
 */
function clientClusterData(redirectSessionId, opt) {
	var flags = ClusterFlags.CC_REDIRECTION_SUPPORTED | (ClusterVersion.CC_REDIRECT_VERSION_4 << 2);
	if (redirectSessionId != 0) {
		flags |= ClusterFlags.CC_REDIRECT_SESSIONID_FIELD_VALID;
	}
	var self = {
		__TYPE__ : MessageType.CS_CLUSTER,
		flags : new type.UInt32Le(flags),
		redirectedSessionId : new type.UInt32Le(redirectSessionId)
	};
