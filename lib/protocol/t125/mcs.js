...
function Client(transport) {
	...
	this.clientClusterData = gcc.clientClusterData(this.redirectSesisonId);
...
/**
 * initialize clientNetworkData for Virtual connection      uchida
 */
Client.prototype.createNetworkData = function() {
	if (this.channels.length <= 1) return;

	var channelDefArray = new type.Component([]);
	for (var i=1; i<this.channels.length; i++) {
		var bn = new Buffer.alloc(8);
		bn.write(this.channels[i].name);

		var cdef = gcc.channelDef();
		cdef.obj.name = new type.BinaryString(bn, { readLength : new type.CallableValue(8) });
		cdef.obj.options = new type.UInt32Le( this.channels[i].flags );
		//cdef.obj.options = new type.UInt32Be( this.channels[i].flags );

		channelDefArray.obj.push( cdef );
	}
	this.clientNetworkData = gcc.clientNetworkData(channelDefArray);
}

Client.prototype.setRedirectSessionId = function(redirectSesisonId) {
	this.redirectSesisonId = redirectSesisonId;
	this.clientClusterData = gcc.clientClusterData(this.redirectSesisonId);
}
...
Client.prototype.recvAttachUserConfirm = function(s) {
    ...
    this.channels.unshift({ id : this.userId, name : 'user' }); // this.channels.push({ id : this.userId, name : 'user' });
...
Client.prototype.recvChannelJoinConfirm = function(s) {
	...
	if (opcode & 2) {
		var joinId = per.readInteger16(s);
	}
...
Client.prototype.sendConnectInitial = function() {
	...
	var ccReq = gcc.writeConferenceCreateRequest(new type.Component([
		...
		//gcc.block(this.clientClusterData)
	])).toStream().getValue();
